...
- **`memory_stats`**: 獲取大腦統計數據（需啟用 `enableManagementTools`）。
- **`memory_list`**: 列出最近的記憶（需啟用 `enableManagementTools`）。

## 3. 自我診斷 (Self-Diagnosis)

如果你發現自己在調用 `memory_store` 或 `memory_recall` 時失敗，或出現權限錯誤（如 `Permission Denied` 或 `profile restriction`）：
1. **立即停止重試**：這是系統層級的權限限制，不是你的邏輯錯誤。
2. **主動告知 User**：「船長，我的工具箱被鎖住了！因為 OpenClaw 3/2 的更新限制，請檢查你的 `openclaw.json` 是否已將我的 `tools.profile` 設定為 `"full"` 並重啟 Gateway。」

## 4. Session 交接機制 (First-Turn Ephemeral Injection)
