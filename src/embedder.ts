 ...
 * Security-hardened: no regex-based env var resolution, input length limits.
 */
...
// Max input length to prevent abuse (most embedding models cap at ~8K tokens)
 ...
 * Only supports the pattern ${VAR_NAME} with strict validation.
 */
...
export class Embedder {
    ...
    private validateInput(text: string): void {
        ...
        if (text.length > MAX_INPUT_LENGTH) {
            throw new Error(`Input text exceeds max length of ${MAX_INPUT_LENGTH} characters`);
        }
    }
    ...
    private async embedMany(texts: string[], task?: string): Promise<number[][]> {
        ...
        texts.forEach((text, index) => {
            if (text && text.trim().length > 0) {
                if (text.length > MAX_INPUT_LENGTH) {
                    throw new Error(`Input text at index ${index} exceeds max length of ${MAX_INPUT_LENGTH}`);
                }
