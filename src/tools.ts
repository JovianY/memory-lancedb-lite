 ...
 * Security-hardened: input length validation on all parameters.
 */
...
function validateText(text: unknown, fieldName: string, maxLen: number): string {
    ...
    if (text.length > maxLen) {
        throw new Error(`${fieldName} exceeds max length of ${maxLen} characters`);
    }
    return text;
}
    ...
    api.registerTool(
        {
            ...
            async execute(_toolCallId, params) {
                const { query: rawQuery, limit = 5, scope, category } = params as {
                    query: string; limit?: number; scope?: string; category?: string;
                };

                try {
                    const query = validateText(rawQuery, "query", MAX_QUERY_LENGTH);
    ...
    api.registerTool(
        {
            ...
            async execute(_toolCallId, params) {
                const { text: rawText, importance = 0.7, category = "other", scope } = params as {
                    text: string; importance?: number; category?: string; scope?: string;
                };

                try {
                    const text = validateText(rawText, "text", MAX_TEXT_LENGTH);
                    ...
                    const entry = await context.store.store({
                        text, vector, importance: safeImportance, category: category as any, scope: targetScope,
                    });
    ...
    api.registerTool(
        {
            ...
            async execute(_toolCallId, params) {
                const { query, memoryId, scope } = params as {
                    query?: string; memoryId?: string; scope?: string;
                };
                ...
                try {
                    ...
                    if (query) {
                        const validatedQuery = validateText(query, "query", MAX_QUERY_LENGTH);
                        const results = await context.retriever.retrieve({ query: validatedQuery, limit: 5, scopeFilter });

                        if (results.length === 0) {
                            return {
                                content: [{ type: "text", text: "No matching memories found." }],
                                details: { found: 0, query: validatedQuery },
                            };
                        }
    ...
    api.registerTool(
        {
            ...
            async execute(_toolCallId, params) {
                ...
                try {
                    ...
                    if (text) {
                        const validatedText = validateText(text, "text", MAX_TEXT_LENGTH);
                        if (isNoise(validatedText)) {
                            return {
                                content: [{ type: "text", text: "Skipped: updated text detected as noise" }],
                                details: { action: "noise_filtered" },
                            };
                        }
                        newVector = await context.embedder.embedPassage(validatedText);
                    }
                    ...
                    return {
                        ...
                        details: {
                            action: "updated", id: updated.id, scope: updated.scope,
                            category: updated.category, importance: updated.importance,
                ...
                try {
                    ...
                    return {
                        ...
                        details: {
                            ...
                            retrievalConfig: { ...retrievalConfig, rerankApiKey: retrievalConfig.rerankApiKey ? "***" : undefined },
                    ...
                    return {
                        ...
                        details: {
                            ...
                            memories: entries.map(e => ({
                                ...
                                scope: e.scope, importance: e.importance, timestamp: e.timestamp,
                            })),
